...
function mergeObjects(objects) {
  const result = objects.map(Object.entries).reduce((acc, currentHalue) => {
    currentHalue.forEach(([key, value]) => {
      /*
...
function makeWord(lettersObject) {
  ...
  entries.forEach(([char, positions]) => {
    positions.forEach((position) => {
      letterArray[position] = char;
    });
  });
...
function sellTickets(queue) {
  const price = 25;
  return (
    queue.reduce((bank, payment) => {
      if (payment === price) {
        return bank + payment;
      }
      return bank - (payment - price);
    }, 0) >= 0
  );
}
...
function group(array, keySelector, valueSelector) {
  return array
    .reduce((acc, item) => {
      ...
      const existingGroup = acc.find(([groupKey]) => groupKey === key);
      if (existingGroup) {
        existingGroup[1].push(valueSelector(item));
      } else {
        acc.push([key, [valueSelector(item)]]);
      }
      return acc;
    }, [])
    .reduce((accMap, [key, values]) => {
